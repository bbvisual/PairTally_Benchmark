...
# Custom Dataset class for DICTA25 data (adapted for Ovis2-16B)
...
def get_args_parser():
    parser = argparse.ArgumentParser("DICTA25 Ovis2-16B Evaluation", add_help=False)
    ...
    parser.add_argument(
        ...
        default="./Ovis2_16B_DICTA25_Results",
    )
    parser.add_argument(
        "--base_data_path",
        help="base path to DICTA25 data",
        default="/home/khanhnguyen/DICTA25",
    )
    parser.add_argument(
        "--dataset_name",
...
def load_ovis2_model(model_id):
    """Load Ovis2-16B model using the official runner"""
    ...
    try:
        ...
        print("Ovis2-16B model loaded successfully!")
        return runner
        
    except Exception as e:
        print(f"Error loading {model_id}: {e}")
        print("Make sure you have installed Ovis:")
        print("  git clone https://github.com/AIDC-AI/Ovis.git")
        print("  cd Ovis")
...
def run_ovis2_inference(runner, image_path, prompt, args):
    """Run inference on a single image with Ovis2-16B"""
    ...
    print(f"    Ovis2-16B inference with simplified count-only prompt: '{counting_prompt}'")
    
    try:
        # Load image
        image = Image.open(image_path).convert('RGB')
        
        # Run inference using Ovis runner
        ...
        print(f"    Ovis2-16B inference failed: {e}")
        raise e
...
def save_qualitative_data(img_id, img_name, num_gt, num_pred, mae_error, class_type, 
                         prompt, response_text, parsing_info, predicted_points, args):
    """Save qualitative data to structured format matching CountGD pattern"""
    
    # Create data structure (adapted for Ovis2-16B - count only, no points)
    data_entry = {
        'image_id': int(img_id),
        'image_name': img_name,
        ...
        'predicted_boxes': [],     # Empty for Ovis2-16B (no box detection)
        'predicted_points': [],    # Empty for simplified version (no point detection)
        'prediction_scores': [],   # Empty for Ovis2-16B (no confidence scores)
        'prompt': prompt,
        'inference_mode': 'ovis2_16B_count_only',
        ...
        'notes': {
            ...
            'ovis2_16B_count_only': True,
            'no_bounding_boxes': True,
            'no_point_detection': True,
            'multimodal_large_language_model': True
        }
    }
...
def save_quantitative_results(all_results, dataset_folder_name, model_name):
    ...
    quant_output_dir = "/home/khanhnguyen/DICTA25-RESULTS/Ovis2_16B-quantitative"
    ...
    with open(summary_file, 'w') as f:
        f.write(f"Ovis2-16B Quantitative Results Summary\n")
        f.write(f"======================================\n\n")
        f.write(f"Dataset: {dataset_folder_name}\n")
        f.write(f"Model: {model_name}\n")
        f.write(f"Inference Mode: Count-Only (Ovis2-16B)\n")
...
def main():
    parser = argparse.ArgumentParser("DICTA25 Ovis2-16B Evaluation", parents=[get_args_parser()])
    ...
    print("Ovis2-16B DICTA25 Evaluation")
    print("============================")
    ...
    # Load Ovis2-16B model
    print("Loading Ovis2-16B model...")
    runner = load_ovis2_model(args.model_id)
    
    # Create output directory structure matching CountGD pattern
    base_output_dir = "/home/khanhnguyen/DICTA25-RESULTS/Ovis2_16B-qualitative"
    ...
    for i in range(limit):
        ...
        pos_response = run_ovis2_inference(runner, image_path, pos_prompt, args)
        ...
        data_entry_pos = save_qualitative_data(
            ...
            'positive', pos_prompt, pos_response, pos_parsing_info, [], args  # No points
        )
        ...
        print(f"  Response: '{pos_response[:100]}...' -> {pos_parsing_info}")
        
        # ===== NEGATIVE CLASS INFERENCE =====
        print(f"  Running negative inference with prompt: '{neg_prompt}'")
        neg_response = run_ovis2_inference(runner, image_path, neg_prompt, args)
        ...
        data_entry_neg = save_qualitative_data(
            ...
            'negative', neg_prompt, neg_response, neg_parsing_info, [], args  # No points
        )
        ...
        print(f"  Response: '{neg_response[:100]}...' -> {neg_parsing_info}")
    ...
    save_quantitative_results(all_results, dataset_folder_name, 'Ovis2_16B')
    
    print(f"\n=== Final Summary ===")
    print(f"Dataset folder: {dataset_folder_name}")
    print(f"Model: Ovis2-16B")
    print(f"Qualitative results saved to: {dataset_output_dir}")
    print(f"Quantitative results saved to: /home/khanhnguyen/DICTA25-RESULTS/Ovis2_16B-quantitative/{dataset_folder_name}/")
    ...
    print("\nOvis2-16B evaluation completed successfully!")
