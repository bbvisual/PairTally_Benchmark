...
# Custom Dataset class for DICTA25 data (adapted for Qwen2.5-VL)
...
def get_args_parser():
    parser = argparse.ArgumentParser("DICTA25 Qwen2.5-VL Evaluation", add_help=False)
...
def run_qwen2_5vl_inference(model, processor, image_path, prompt, args):
    """Run inference on a single image with Qwen2.5-VL"""
    ...
    print(f"    Qwen2.5-VL inference with simplified count-only prompt: '{counting_prompt}'")
...
def save_qualitative_data(img_id, img_name, num_gt, num_pred, mae_error, class_type, 
    ...
    # Create data structure (adapted for Qwen2.5-VL - count only, no points)
    data_entry = {
        'image_id': int(img_id),
        'image_name': img_name,
        ...
        'predicted_boxes': [],     # Empty for Qwen2.5-VL (no box detection)
        'predicted_points': [],    # Empty for simplified version (no point detection)
        'prediction_scores': [],   # Empty for Qwen2.5-VL (no confidence scores)
        'prompt': prompt,
        'inference_mode': 'qwen2_5vl_count_only',
        ...
        'notes': {
            ...
            'qwen2_5vl_count_only': True,
            'no_bounding_boxes': True,
            'no_point_detection': True,
            'vision_language_model_count_only': True
        }
    }
...
def save_quantitative_results(all_results, dataset_folder_name, model_name):
    ...
    quant_output_dir = "/home/khanhnguyen/DICTA25-RESULTS/Qwen2_5VL-quantitative"
    ...
    with open(summary_file, 'w') as f:
        f.write(f"Qwen2.5-VL Quantitative Results Summary\n")
        f.write(f"======================================\n\n")
        f.write(f"Dataset: {dataset_folder_name}\n")
        f.write(f"Model: {model_name}\n")
        f.write(f"Inference Mode: Count-Only (Qwen2.5-VL)\n")
...
def main():
    parser = argparse.ArgumentParser("DICTA25 Qwen2.5-VL Evaluation", parents=[get_args_parser()])
    ...
    print("Qwen2.5-VL DICTA25 Evaluation")
    print("=============================")
    ...
    base_output_dir = "/home/khanhnguyen/DICTA25-RESULTS/Qwen2_5VL-qualitative"
    ...
    for i in range(limit):
        ...
        pos_response = run_qwen2_5vl_inference(model, processor, image_path, pos_prompt, args)
        ...
        neg_response = run_qwen2_5vl_inference(model, processor, image_path, neg_prompt, args)
    ...
    save_quantitative_results(all_results, dataset_folder_name, 'Qwen2_5VL')
    ...
    print(f"Quantitative results saved to: /home/khanhnguyen/DICTA25-RESULTS/Qwen2_5VL-quantitative/{dataset_folder_name}/")
    ...
    print("\nQwen2.5-VL evaluation completed successfully!")
