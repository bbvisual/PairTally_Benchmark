...
# Custom Dataset class for PairTally data (adapted for InternVL3 Combined)
...
def get_args_parser():
    parser = argparse.ArgumentParser("PairTally InternVL3 Combined Evaluation", add_help=False)
    ...
    parser.add_argument(
        ...
        default="./InternVL_PairTally_Results",
    )
    parser.add_argument(
        "--base_data_path",
...
def run_internvl3_combined_inference(model, tokenizer, image_path, combined_prompt, args):
    ...
    question = f"""<image>
Count the number of {combined_prompt} in this image. Provide only the total count in this format: <count>N</count>. If you see no {combined_prompt} or are unsure, respond with <count>0</count>."""
    ...
    try:
        ...
        # Create generation config with standardized parameters
...
def main():
    parser = argparse.ArgumentParser("PairTally InternVL3 Combined Evaluation", parents=[get_args_parser()])
    ...
    print("InternVL3 PairTally Combined Evaluation")
    print("========================================")
