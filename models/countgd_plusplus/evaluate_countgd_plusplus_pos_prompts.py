...
class CountGDPlusPlus:
    ...
    def predict(self, image, pos_exemplars, neg_exemplars, pos_prompt, neg_prompt):
        """
        Predict count for given image and only positive text and exemplar prompts
        
        Args:
            image: PIL Image
            pos_exemplars: List of bounding boxes corresponding to the object to count
            neg_exemplars: List of bounding boxes corresponding to the object to NOT count (unused by model)
            pos_prompt: Text prompt corresponding to the object to count
            neg_prompt: Text prompt corresponding to the object to NOT count (unused by model)
        ...
        caption = pos_prompt + " ." # caption only contains positive prompt
        ...
        with torch.no_grad():
            model_output = self.model(
                ...
                [], # no negative exemplar boxes provided
                captions=[caption],
            )
...
def evaluate_model(config):
    ...
    for image_name, annotation in tqdm(annotations.items()):
        try:
            ...
            # Predict count
            """
            NOTE: Negative prompts dropped in model [predict] function
            """
...
if __name__ == "__main__":
    # Example usage
    print("CountGD++ PairTally evaluation (given positive text and 3 positive exemplars)")
