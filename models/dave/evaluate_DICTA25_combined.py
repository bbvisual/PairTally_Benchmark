...
def save_qualitative_data(img_tensor, combined_bboxes, density_map_pred, pred_bboxes, pred_scores,
                         img_id, img_name, pos_count, neg_count, total_count, pred_count, mae_error, output_dir):
...
def save_quantitative_results(all_results, dataset_folder_name, model_name):
    ...
    quant_output_dir = "/home/khanhnguyen/DICTA25-RESULTS/DAVE-quantitative-combined"
    ...
    if total_images > 0:
        ...
        with open(summary_file, 'w') as f:
            f.write(f"DAVE Combined Inference Results Summary\n")
            f.write(f"======================================\n\n")
...
def evaluate_combined(args):
    ...
    base_output_dir = "/home/khanhnguyen/DICTA25-RESULTS/DAVE-qualitative-combined"
    ...
    for batch_idx, batch_data in enumerate(tqdm(dataloader, desc="Processing images")):
        ...
        if len(positive_exemplars) >= 2 and len(negative_exemplars) >= 1:
            ...
            # Run inference with combined exemplars - DAVE returns (density_map, aux_outputs, tblr, bboxes)
            outputs_R, aux_outputs, tblr, pred_bboxes = model(img, combined_bboxes)
            
            # Extract density map and predicted bboxes
            density_map_pred = outputs_R
            
            # Extract bounding boxes and scores from DAVE's BoxList format
            if hasattr(pred_bboxes, 'box'):
                pred_boxes_tensor = pred_bboxes.box
                pred_scores_tensor = pred_bboxes.fields.get('scores', torch.zeros(len(pred_boxes_tensor)))
            else:
                pred_boxes_tensor = torch.zeros((0, 4))
                pred_scores_tensor = torch.zeros((0,))
            
            # Calculate count and error against total ground truth
            pred_count = len(pred_boxes_tensor) if len(pred_boxes_tensor) > 0 else float(density_map_pred.sum().item())
    ...
    print(f"Quantitative results saved to: /home/khanhnguyen/DICTA25-RESULTS/DAVE-quantitative-combined/{dataset_folder_name}/")
    print(f"\nFiles created:")
    print(f"  - combined_inference_data.json")
    print(f"  - combined_inference_data.pkl")

if __name__ == '__main__':
    parser = argparse.ArgumentParser('DAVE Combined Inference', parents=[get_argparser()])
    ...
    print("Starting DAVE combined inference (2 positive + 1 negative exemplars)...")
    print(f"Model: {args.model_name}")
    print(f"Annotation file: {args.annotation_file}")
    print(f"Image directory: {args.image_dir}")
    
    evaluate_combined(args)
    print("\nDAVE combined inference completed!")
