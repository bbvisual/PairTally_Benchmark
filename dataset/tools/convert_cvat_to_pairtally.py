#!/usr/bin/env python3
"""
Conversion script for CVAT annotations to PairTally format.
Converts parsed_annotations.json (CVAT format) to:
1. pairtally_annotations.json (all images)
2. Train_Test_Val_FSC_147.json (all images)
3. pairtally_annotations_inter.json (INTER objects only)
4. Train_Test_Val_FSC_147_inter.json (INTER objects only)
5. pairtally_annotations_intra.json (INTRA objects only)
6. Train_Test_Val_FSC_147_intra.json (INTRA objects only)

Optional (with --create_super_category_files flag):
7. pairtally_annotations_{category}.json (per super category: HOU, OTR, FOO, etc.)
...
def convert_cvat_to_fsc147(cvat_data, mapping_strategy='compact'):
    """
    Convert CVAT annotation data to PairTally annotation format with compact filenames.
    ...
    for original_filename, annotations in cvat_data.items():
        ...
        # Create PairTally entry using compact filename as key
        fsc147_entry = {
            "points": points,
...
def main():
    parser = argparse.ArgumentParser(description='Convert CVAT annotations to PairTally format')
    ...
    parser.add_argument('--annotation_file', default='pairtally_annotations.json', 
    ...
    parser.add_argument('--inter_annotation_file', default='pairtally_annotations_inter.json',
                       help='Output annotation file name for inter objects only')
    parser.add_argument('--intra_annotation_file', default='pairtally_annotations_intra.json',
    ...
    # Convert to PairTally annotation format
    print(f"Converting to PairTally annotation format using '{args.mapping_strategy}' filename mapping...")
    ...
    # Use new compact filenames from the PairTally annotations
    ...
    if args.create_super_category_files:
        ...
        for category in unique_categories:
            ...
            # Filter PairTally annotations by super category using metadata
            ...
            if category_fsc147_data:
                ...
                category_annotation_file = f'pairtally_annotations_{category.lower()}.json'
